...
import { ActionIcons, GESTURE_ICONS, getGestureIcon } from '@/components/icons/gesture-icons';
...
export function AddGestureModal() {
  ...
  const [selectedIconId, setSelectedIconId] = useState(GESTURE_ICONS[0].id);
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      setStep(1);
      setName('');
      setSelectedIconId(GESTURE_ICONS[0].id);
      setSelectedAction('');
      setIsRecording(false);
      setSampleCount(0);
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-6 gap-2">
                        {GESTURE_ICONS.map((g) => (
                          <motion.button
                            key={g.id}
                            onClick={() => setSelectedIconId(g.id)}
                            className={`p-2.5 rounded-lg transition-smooth ${selectedIconId === g.id
                            ...
                            title={g.label}
                          >
                            <div className="w-8 h-8 mx-auto text-indigo-300">
                              {g.icon}
                            </div>
                          </motion.button>
                        ))}
                  ...
                  >
                    ...
                    <div className="w-full glass-card border border-white/15 rounded-xl p-4 space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 text-indigo-300">
                          {getGestureIcon(selectedIconId)}
                        </div>
                        <div>
